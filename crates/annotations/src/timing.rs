...
impl AnnotationTiming {
    ...
    pub fn for_tool(tool: Tool, start: f64, scene_duration: f64) -> Self {
        // A click on the last frame must still produce an editable interval.
        let start = start.clamp(0.0, (scene_duration - Self::MINIMUM_DURATION).max(0.0));
        let end = (start + Self::DEFAULT_DURATION).min(scene_duration.max(0.0));
    ...
    /// Nearest frame with both transitions complete. Keep an already useful
    /// playhead in place; short regions meet at their midpoint.
    pub fn editing_time(&self, current: f64) -> f64 {
        let transition = self.transition.clamp(0.0, self.duration() * 0.5).max(1e-6);
        let first = if self.entrance == EntranceEffect::None {
            self.start
        } else {
            self.start + transition
        };
        let last = if self.exit == ExitEffect::None {
            self.end
        } else {
            self.end - transition
        };
...
/// An editing preview shows the entire mark, including a draft created on
/// the last frame. Callers enable this only while authoring; scrubbing and
/// playback must show the same result as export.
    ...
    if progress < 1.0 {
        match (timing.entrance, mark.tool) {
            ...
            (EntranceEffect::Draw, Tool::Line | Tool::Arrow) => {
                if mark.hand_drawn || mark.bend.abs() > 0.00001 {
                    animated.draw_progress = Some(progress);
                } else {
...
fn scale_mark(mark: &mut AnnotationMark, scale: f32) {
    ...
    if mark.tool == Tool::Text {
        ...
        mark.scale_stroke_width(scale.max(0.3));
    }
}
...
mod tests {
    ...
    #[test]
    fn only_live_edits_bypass_animation_and_time_range() {
        for tool in [Tool::Rectangle, Tool::Text] {
            let mark = mark(tool);
            assert!(editor_mark(&mark, 0.5, false).is_none());
            assert!(editor_mark(&mark, 3.5, false).is_none());
            assert_eq!(editor_mark(&mark, 3.5, true), Some(mark.clone()));
    ...
    #[test]
    fn editing_time_is_fully_visible_for_every_effect_even_at_the_last_frame() {
        for duration in [0.1, 0.2, 0.4, 2.5, 10.0] {
            for start in [0.0, duration - 0.05, duration, duration + 3.0] {
                for entrance in EntranceEffect::ALL {
                    for exit in ExitEffect::ALL {
                        let mut timing = AnnotationTiming::for_tool(Tool::Text, start, duration);
                        timing.entrance = entrance;
                        timing.exit = exit;
                        assert!(timing.start >= 0.0 && timing.end <= duration + 1e-9);
                        let time = timing.editing_time(start);
                        assert!((0.0..=duration).contains(&time));
                        let state = timing.state_at(time);
                        assert!(state.visible);
                        assert!((state.opacity - 1.0).abs() < 1e-5, "{timing:?} {state:?}");
                        assert!((state.progress - 1.0).abs() < 1e-5);
                        assert!((state.scale - 1.0).abs() < 1e-5);
                        assert_eq!(timing.editing_time(time), time);
                    }
                }
            }
        }
        let timing = AnnotationTiming::default();
        assert_eq!(
            timing.editing_time(1.0),
            1.0,
            "leave useful playheads alone"
        );
    }
}

/// Express a pinned mark within the visible normalized source rectangle.
pub fn in_visible_media_space(
    mark: AnnotationMark,
    ...
    let (left, top, visible_x, visible_y) = visible;
    let mut mapped = mark;
    ...
    mapped.scale_stroke_width(visible_y as f32);
    mapped
}
